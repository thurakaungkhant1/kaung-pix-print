CREATE OR REPLACE FUNCTION public.purchase_product_wallet(p_product_id bigint, p_quantity integer DEFAULT 1, p_game_id text DEFAULT NULL::text, p_server_id text DEFAULT NULL::text, p_phone_number text DEFAULT NULL::text, p_plan_id uuid DEFAULT NULL::uuid, p_plan_name text DEFAULT NULL::text, p_delivery_address text DEFAULT ''::text, p_player_name text DEFAULT NULL::text)
...
BEGIN
  ...
  v_is_game := NULLIF(btrim(COALESCE(v_product.smile_package_id, '')), '') IS NOT NULL;
  v_plan_name := p_plan_name;
  v_smile_id := NULL;

  IF v_is_game THEN
    v_smile_id := btrim(v_product.smile_package_id);
    v_plan_name := COALESCE(p_plan_name, v_product.name);
  END IF;

  v_player := NULLIF(btrim(COALESCE(p_player_name, '')), '');
  v_game_name := v_product.category;
  IF v_player IS NOT NULL THEN
    v_game_name := v_product.category || ' (' || left(v_player, 60) || ')';
  END IF;
  ...
  INSERT INTO public.orders (
    ...
    plan_id, plan_name, smile_package_id,
    status, payment_method, delivery_address
  ) VALUES (
    v_user, p_product_id, v_qty, v_total,
    p_game_id, p_server_id, v_game_name, COALESCE(p_phone_number,''),
    p_plan_id, v_plan_name, v_smile_id,
  ...
  VALUES
    (v_user, -v_total, 'purchase', v_order_id,
     'Purchase: ' || COALESCE(v_product.name,'order') ||
       CASE WHEN v_plan_name IS NOT NULL THEN ' — ' || v_plan_name ELSE '' END ||
       ' x' || v_qty,
     v_new_balance);
