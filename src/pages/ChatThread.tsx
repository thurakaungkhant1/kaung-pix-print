...
const ChatThread = () => {
  ...
  return (
    <MobileLayout className="flex flex-col bg-background">
      ...
      <div
        className="flex-1 overflow-y-auto p-4 space-y-1.5 pb-4 relative"
        style={{
          backgroundImage:
            "radial-gradient(circle at 20% 10%, hsl(var(--primary)/0.08), transparent 40%), radial-gradient(circle at 80% 90%, hsl(var(--primary)/0.06), transparent 45%), linear-gradient(to bottom, hsl(var(--background)), hsl(var(--muted)/0.25))",
        }}
      >
        {messages.length === 0 && (
          <div className="text-center py-20 px-6">
            <div className="w-20 h-20 mx-auto rounded-3xl bg-gradient-to-br from-primary/20 to-primary/5 border border-primary/20 flex items-center justify-center mb-4 shadow-inner">
              <Smile className="h-9 w-9 text-primary" />
            </div>
            <h3 className="font-display font-semibold text-lg mb-1">မင်္ဂလာပါ 👋</h3>
        ...
        {messages.map((m, idx) => {
          ...
          return (
            <div key={m.id} className={`flex ${mine ? "justify-end" : "justify-start"} ${grouped ? "mt-0.5" : "mt-2"}`}>
              <div
                className={`max-w-[78%] px-3.5 py-2 text-sm leading-relaxed break-words shadow-sm ${
                  mine
                    ? `bg-gradient-to-br from-primary to-primary/85 text-primary-foreground rounded-2xl ${grouped ? "rounded-br-2xl" : "rounded-br-md"}`
                    : `bg-card/95 backdrop-blur border border-border/60 text-card-foreground rounded-2xl ${grouped ? "rounded-bl-2xl" : "rounded-bl-md"}`
                }`}
              >
                <p className="whitespace-pre-wrap">{m.content}</p>
                <p className={`text-[10px] mt-1 flex items-center gap-1 ${mine ? "justify-end opacity-90" : "text-muted-foreground"}`}>
        ...
        {otherTyping && (
          <div className="flex justify-start mt-2">
            <div className="bg-card/95 backdrop-blur border border-border/60 rounded-2xl rounded-bl-md px-4 py-2.5 flex items-center gap-1">
