...
import { ArrowLeft, Crown, Calendar, Coins, History } from "lucide-react";
...
const PremiumHistory = () => {
  ...
  const loadData = async () => {
    ...
    // Load premium-related transactions
    const { data: txData } = await supabase
      .from("point_transactions")
      .select("*")
      .eq("user_id", user.id)
      .or("transaction_type.eq.chat_reward,transaction_type.eq.premium_purchase")
                  ...
                  {transactions.map((tx) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className={cn(
                          "p-2 rounded-lg",
                          tx.transaction_type === "chat_reward" 
                            ? "bg-green-500/10" 
                            : "bg-amber-500/10"
                        )}>
                          {tx.transaction_type === "chat_reward" ? (
                        ...
                        <div>
                          <p className="font-medium text-sm">
                            {tx.transaction_type === "chat_reward" ? "Chat Reward" : "Premium Purchase"}
                          </p>
