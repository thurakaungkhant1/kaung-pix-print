...
import { ArrowLeft, Plus, Pencil, Trash2, Gamepad2 } from "lucide-react";
import {
...
const PubgUcPackages = () => {
  const [packages, setPackages] = useState<PubgPackage[]>([]);
  ...
  const handleDelete = async (id: number) => {
    ...
    if (error) {
      ...
      toast({ title: "Package deleted", description: "PUBG UC package has been deleted successfully" });
      loadPackages();
    }
  };
      ...
      <header className="bg-gradient-primary text-primary-foreground p-4 sticky top-0 z-40">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold">PUBG UC Packages</h1>
          </div>
        </div>
      </header>
        ...
        <Card>
          <CardHeader>
            <div className="flex justify-between items-center">
              <CardTitle>Manage PUBG Mobile UC Packages</CardTitle>
              <Button onClick={() => navigate("/admin/pubg-uc-packages/new")}>
          ...
          <CardContent>
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-muted-foreground mb-4">No PUBG UC packages yet</p>
                <Button onClick={() => navigate("/admin/pubg-uc-packages/new")}>
                  <Plus className="h-4 w-4 mr-2" />
                  Add First Package
                </Button>
                    ...
                    <TableRow key={pkg.id}>
                      <TableCell>
                        <div className="flex items-center gap-3">
                          <img src={pkg.image_url} alt={pkg.name} className="h-12 w-12 rounded object-cover" />
                          <div>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1">
                          ...
                          <span className="font-medium">{pkg.name.match(/\d+/)?.[0] || "-"}</span>
                        </div>
                      </TableCell>
                      <TableCell>{pkg.points_value}</TableCell>
                      <TableCell>{new Date(pkg.created_at).toLocaleDateString()}</TableCell>
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2">
                          <Button variant="outline" size="sm" onClick={() => navigate(`/admin/pubg-uc-packages/edit/${pkg.id}`)}>
